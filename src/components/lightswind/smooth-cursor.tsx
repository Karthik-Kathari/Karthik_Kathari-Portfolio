...
export function SmoothCursor({
  ...
  useEffect(() => {
    ...
    const updateTrail = (pos: { x: number; y: number }, isInteractive: boolean) => {
      if (!showTrail || isInteractive) return;
    ...
    const findMagneticElement = (x: number, y: number) => {
      ...
      for (const element of Array.from(elements)) {
        if ((element as HTMLElement).dataset.noMagnetic === "true") continue;
    ...
    const smoothMouseMove = (e: MouseEvent) => {
      ...
      updateTrail(currentPos, isInteractive);
